    ...
    SELECT
        ...
        CASE WHEN (EXTRACT(isodow FROM (now() - INTERVAL '32 hours')::date) IN (6, 7)) THEN 2
             WHEN ((SELECT count(*) FROM eod_v1 WHERE event_date=(select (now() - INTERVAL '32 hours')::date)) > 35000 AND
                   (SELECT count(*) FROM seeking_alpha_v1 WHERE event_date=(select (now() - INTERVAL '32 hours')::date)) > 3500 AND
                   (SELECT count(*) FROM zacks_financials_v1 WHERE event_date=(select (now() - INTERVAL '32 hours')::date)) > 3500) THEN 1
             ELSE 0
        END AS status,
